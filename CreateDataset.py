...
def main():
	...
	dataset_pandas.to_csv("not_norm.csv", index=False)
...
def get_sample_arrays(dataset_dir, folder_name, samp_rate):
	...
	for audio in path_of_audios:
		...
		x, sr = librosa.load(audio, sr=samp_rate, offset=0.0, duration=5.0)
		y, sr = librosa.load(audio, sr=samp_rate, offset=12.0, duration=5.0)
		z, sr = librosa.load(audio, sr=samp_rate, offset=24.0, duration=5.0)
...
def avg(ax, ay, az):
	#print(ax.dtype)
	numpy.savetxt("ax.csv", ax, delimiter=",")
	#print(az.dtype)
	numpy.savetxt("az.csv", az, delimiter=",")
	
	numpy.savetxt("ay.csv", ay, delimiter=",")
	...
	j=0
	final = []
	for row1 in rx and row2 in ry and row3 in rz:
		final.append([])
		for col in row:
			final[i][j].append((float(row1[j])+float(row2[j])+float(row3[j]))/3)
			j=j+1
		i=i+1
